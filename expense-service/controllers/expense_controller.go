...
func CreateExpense(c *gin.Context) {
	paidBy := getUserID(c)
	if paidBy == "" {
		paidBy = "uuid-creador-desconocido" // Fake fallback for unit testing pure endpoint behavior
	}
	...
	for _, s := range req.Splits {
		...
		paidStatus := false
		if s.UserID == paidBy {
			// Lo que pagué por mi propia parte ya lo pagué de una vez ("paid"=true simulado en la lógica split normal)
			// Nos sirve para no quedar adeudándonos a nosotros mismos.
			// Implementamos logica general de base falsa pa deuda normal.
		}
...
func DeleteExpense(c *gin.Context) {
	// (Simulando un admin_check ideal delegable o verificado manual)
	// Para uso real se validari un X-Role de gateway.
	id := c.Param("id")
